...
gridExtra::grid.arrange(g_Tmin, g_Tavg, g_Tmax, g_Depart, g_DewPoint, g_WetBulb, nrow=1)
...
fit_logit <- caret::train(WnvPresentF ~ Latitude + Longitude + Station + Block + Year + Month + Tavg + DewPoint + Species, data=df_train_wthr, method="glm", family="binomial", trControl = train_ctrl, metric = "kappa")
# - print model
fit_logit
summary(fit_logit$finalModel)
# - area under roc curve
pred_logit_train <- predict(fit_logit, newdata=df_train_wthr, type="prob")[,"present"]
Metrics::auc(df_train_wthr$WnvPresent, pred_logit_train)
# - accuracy 
caret::confusionMatrix(df_train_wthr$WnvPresentF, predict(fit_logit, newdata=df_train_wthr))


# - create submission file (this should be udf)
df_submission <- load_submission(path_data_submission)
df_submission$WnvPresent <- predict(pred_logit_train, newdata=df_test_wthr, type="prob")[,"present"]
...
# - values with NA will cause predict to return less results!
fit_gbm <- train(WnvPresentF ~ Latitude+Longitude+Species+Block+Year+Month+Day+Tmax+Tmin+Tavg+DewPoint+Heat+Cool+SeaLevel+ResultSpeed+ResultDir+AvgSpeed+Elevation, method="gbm", data=df_train_fit_regs, verbose=F)
fit_gbm
fit_gbm$finalModel
...
path_submit <- file.path(dir_submissions, "7_gbm_addl_vars.csv")
readr::write_csv(df_submission, path_submit)
