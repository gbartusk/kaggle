...
prcomp(df_train_temp, center=T, scale=T); summary(prcomp(df_train_temp, center=T, scale=T))
pca_temp_obj <- caret::preProcess(df_train_temp, thresh=0.95, method=c("BoxCox", "center", "scale", "pca"))
pred_pca_temp <- predict(pca_temp_obj, df_train_temp)
...
fit_rpart <- train(WnvPresentF ~ ., method="rpart", data=df_train_fit_regs)
fit$finalModel
...
# - fit: train 
fit_rf <- train(WnvPresentF ~ ., method="rf", data=df_fit, importance=TRUE)
...
fit_rf_all <- train(WnvPresentF ~ ., method="rf", data=df_train_all, importance=TRUE)
