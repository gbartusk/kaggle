...
# - validation set
set.seed(123)
train_index <- caret::createDataPartition(y=df_train_wthr$WnvPresent, p=0.8, list=FALSE)
...
# - add weather based pca - ** FIX - CAN ONLY FIX PCs ON TRAINING! **
df_train_wthr <- get_weather_pcas(df_train_wthr)
df_test_wthr <- get_weather_pcas(df_test_wthr)
df_train_wthr_fit <- get_weather_pcas(df_train_wthr_fit)
df_train_wthr_val <- get_weather_pcas(df_train_wthr_val)
...
fit_rpart <- rpart(WnvPresent ~ Latitude + Latitude + Block + Month + PC1_temp + PC2_temp + PC1_pres_lvl + PC1_speed + PrecipTotalLog, data=df_train_wthr, method="class", control=rpart.control(minsplit=34, cp=0, xval=5))
fit_rpart
plot(fit_rpart)
text(fit_rpart)
...
# - lots of outliers - apply log transform to normalize
g_PrecipTotal
ggplot(data=df_train_wthr, aes(x=PrecipTotal, colour=WnvPresentF)) + geom_density()
ggplot(data=df_train_wthr, aes(x=log(PrecipTotal+.001), colour=WnvPresentF)) + geom_density()
...
# - fit: train
set.seed(123)
fit_logit <- glm(WnvPresentF ~ Latitude*Longitude + Species + PC1_temp*PrecipTotalLog + Month + WkDay + PC1_pres_lvl + SunriseNum, family="binomial", weights=Weight, data=df_train_wthr_fit)
# - print model
summary(fit_logit)
...
# - fit: train + vaidation
fit_logit_all <- glm(WnvPresentF ~ Latitude*Longitude + Species + PC1_temp*PrecipTotalLog + Month + WkDay + PC1_pres_lvl + SunriseNum, family="binomial", weights=Weight, data=df_train_wthr)
...
df_train_all <- df_train_wthr %>% dplyr::select(-WnvPresent, -Station, -Depart, -Address, -Street, -AddressNumberAndStreet, -AddressAccuracy, -DateStr, -Trap, -Date, -SatelliteTrap, -NumMosquitos, PrecipTotal)

df_fit <- df_train_wthr_fit %>% dplyr::select(-WnvPresent, -Station, -Depart, -Address, -Street, -AddressNumberAndStreet, -AddressAccuracy, -DateStr, -Trap, -Date, -SatelliteTrap, -NumMosquitos, PrecipTotal)
...
fit_rpart <- train(WnvPresentF ~ ., method="rpart", data=df_train_wthr)
fit_rpart$finalModel
# - plot tree ... doesnt work with one break
rattle::fancyRpartPlot(fit_rpart$finalModel)
...
fit_gbm <- caret::train(WnvPresentF ~ Latitude + Longitude + Species + Year + PC1_temp + PC2_temp + PC1_speed + Sunrise, data=df_fit, method="gbm", verbose=F)

# - relative influence
summary(fit_gbm)

# - roc / accuracy: train
Metrics::auc(df_train_wthr_fit$WnvPresent, predict(fit_gbm, newdata=df_fit, type="prob")[,"present"])
caret::confusionMatrix(df_fit$WnvPresentF, predict(fit_gbm, newdata=df_fit))
...
# - fit: train + validation 
fit_gbm_all <- caret::train(WnvPresentF ~ Latitude + Longitude + Species + Block + Year + Month + WkDay + PrecipTotalLog + PC1_temp + PC2_temp + PC1_pres_lvl + PC1_speed, data=df_train_all, method="gbm", verbose=F)
