...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# - BOOSTING WITH TREES (submission 4)
...
fit_gbm <- train(WnvPresentF ~ Latitude + Longitude + Station + Block + Year + Month + Tavg + DewPoint + Species, method="gbm", data=df_train_fit_regs, verbose=F)
fit_gbm
fit_gbm$finalModel

# - predict on both test and training
pred_test_gbm <- predict(fit_gbm, newdata=df_test_wthr, type="prob"); dim(pred_test_gbm); dim(df_test_wthr)
pred_train_gbm <- predict(fit_gbm, newdata=df_train_fit_regs, type="prob"); dim(pred_train_gbm); dim(df_train_fit_regs)

# - area under roc curve
Metrics::auc(df_train_wthr$WnvPresent, pred_train_gbm[,"present"])

# - accuracy 
caret::confusionMatrix(df_train_wthr$WnvPresentF, predict(fit_gbm, newdata=df_train_fit_regs))

# - create submission file (this should be udf)
df_submission <- load_submission(path_data_submission)
df_submission$WnvPresent <- pred_test_gbm[,"present"]
...
fit_rf <- train(WnvPresentF ~ Latitude + Longitude + Month + Tavg + DewPoint + Species, method="rf", data=df_train_fit_regs)
