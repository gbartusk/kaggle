...
#   > random sub-sampling: caret::createDataPartition()
#   > leave one out (LOOCV): k = n
#   > k-folds: large k (less bias, more variance), small k (more bias, less variance)
#   > bootstrap: 
#   > 0.632 bootstrap: correct underestimation

# - cross validate date
#df_train_fit <- df_train_wthr %>% dplyr::filter(Year != 2011) 
#df_train_xVal <- df_train_wthr %>% dplyr::filter(Year == 2011) 

# - validation set approach
train_index <- caret::createDataPartition(y=df_train_wthr$WnvPresent, p=0.6, list=FALSE, times=1)    # could also use base::sample()
df_train_fit <- df_train_wthr[train_index,]
df_train_xVal <-  df_train_wthr[-train_index,]
dim(df_train_fit); dim(df_train_xVal)
rm(train_index)

# - cross-validation: leave-one-out

# - cross-validation: k-folds
folds <- caret::createFolds(y=df_train_wthr$WnvPresent, k=10, list=TRUE, returnTrain=TRUE)
sapply(folds, length)

# - cross-validation: resampling
folds <- caret::createResample(y=df_train_wthr$WnvPresent, times=10, list=TRUE)
sapply(folds, length)
...
# - fit model 
#set.seed(123)
reg_form <- "WnvPresent ~ Latitude + Longitude + Station + Species + Block + Year + Month + Tavg + DewPoint"
fit <- glm(reg_form, data=df_train_fit, family="binomial")
summary(fit)
#set.seed(123)
#fit2 <- caret::train(WnvPresent ~ Month, data=df_train_fit, method="glm", family="binomial")
#fit2$finalModel
...
Metrics::auc(df_train_xVal$WnvPresent, pred_xVal)

# - confusion matrix
#   at 0.5 threshold, its predicting nothing to be one!
caret::confusionMatrix(ifelse(pred_xVal>=0.5,1,0), df_train_xVal$WnvPresent)
...
df_submission$WnvPresent <- predict(fit_final, newdata=df_test_wthr, type="response")
path_submit <- file.path(dir_submissions, "2_step_logistic.csv")
readr::write_csv(df_submission, path_submit)


#~~~~~~~~~~~~~~~~~~~~~~
# - use caret::confusionMatrix(...)
# - delete one caret is up and running!!!
# - confusion matrix
check <- 
    data.frame(
