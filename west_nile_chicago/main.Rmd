...
train_index <- caret::createDataPartition(y=df_train_wthr$WnvPresent, p=0.6, list=FALSE, times=1)
...
fit_final <- glm(WnvPresent ~ Month, , data=df_train_wthr, family="binomial")

# - create submission file (this should be udf)
df_submission$WnvPresent <- predict(fit_final, newdata=df_test, type="response")
path_submit <- file.path(dir_submissions, "1_simple_logistic.csv")
